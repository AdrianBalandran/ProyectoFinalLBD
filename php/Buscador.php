    ...
    <p class="resultado">La búsqueda es: <span class="color"><?php echo $busca ?> </span></p>
        ...
        <div class="colocacion">
            ...
            function datos($conexion, $busca)
            {
                ...
                $sql = "SELECT FI.ID_FILME, FI.TIPO_FILME TIPO, FI.NOMBRE NOMBRE, FI.IMAGEN IMAGEN, FI.CLASIFICACION CLASIFICACION, FI.FECHA_ESTRENO FECHA_ESTRENO FROM FILME FI WHERE LOWER(FI.NOMBRE) LIKE LOWER('$busca') OR LOWER(FI.NOMBRE) LIKE CONCAT('%',SUBSTR(LOWER('$busca'), 1, 2),'%') OR LOWER(FI.NOMBRE) LIKE CONCAT('%',LOWER('$busca'),'%') OR LOWER(FI.NOMBRE) LIKE CONCAT('%',SUBSTR(LOWER('$busca'), 1, 1),'%');";
