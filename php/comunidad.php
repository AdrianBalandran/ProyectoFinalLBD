...
<?php 
    ...
    function datos($conexion){
        $sql = "SELECT 
            ...
            VI.ID_FILME AS FILME, 
            VI.CALIFICACION AS CAL, 
            VI.FECHA_VISUALIZACION AS VISU, 
            VI.OPINION AS OPINION, 
            VI.ID_VISUALIZACION AS IDVIS,
            PLA.NOMBRE AS NOMPLA, 
            IDI.NOMBRE AS NOMIDI, 
            FILME.NOMBRE AS NOMFILM, 
            FILME.FECHA_ESTRENO AS ESTRENO, 
            FILME.DESCRIPCION AS DES, 
            (SELECT COUNT(*) FROM Megusta WHERE ID_VISUAL = VI.ID_VISUALIZACION) AS NUM_LIKES, 
            IDIOMA.NOMBRE AS IDIOMA, 
            VI.ID_FILME_SE AS ID, 
            VI.TEMPORADA AS TEMP
            FROM VISUALIZACION VI
            JOIN USUARIO USU ON VI.ID_USUARIO = USU.ID_USUARIO
            JOIN PLATAFORMA PLA ON PLA.ID_PLAT = VI.ID_PLATAFORMA
            JOIN IDIOMA IDI ON IDI.ID_IDIOMA = VI.ID_IDIOMA
            JOIN FILME ON FILME.ID_FILME = VI.ID_FILME
            JOIN IDIOMA IDIOMA ON IDIOMA.ID_IDIOMA = VI.ID_IDIOMA
